...
// NOTE: All streaming goes through API_BASE (HF backend → Google Drive)
// Telegram bot uploads to Drive; backend proxies stream with Range support
    ...
    if (input) {
        input.addEventListener('input', () => {
            ...
            searchTimer = setTimeout(async () => {
                ...
                try {
                    const res = await fetch(`${META_BASE}/search?query=${encodeURIComponent(q)}`);
...
async function loadContent(page = 1) {
    ...
    try {
        // ── Load from user's Google Drive library (uploaded by Telegram bot) ──
        const res = await fetch(`${API_BASE}/api/videos`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        
        const rawFiles = (data.videos || []).map(item => enrichFile({
            id: item.id,
            name: item.name,
            clean_name: item.title || item.name,
            year: item.year || '',
            media_type: item.type === 'series' ? 'show' : 'movie',
            overview: item.synopsis || '',
            rating: item.rating || 0,
            poster_url: item.poster_url || item.thumbnail_url || '',
            backdrop_url: item.backdrop_url || item.poster_url || item.thumbnail_url || '',
        ...
        if (page === 1) {
            ...
            allFiles = [...allFiles, ...rawFiles];
        }

        allCollections = buildCollections(allFiles);
        filteredFiles = [...allCollections];

        document.getElementById('content-count').textContent = `${allFiles.length} titles in your library`;
        renderCollectionsRow();
        renderGrid(allCollections);
        if (page === 1) setupBillboard();
        ...
        toast('⚠️ Failed to load library. Is the backend running?', 'error');
    }
}
...
function openDetail(file) {
    ...
    const isTv = file.media_type === 'tv' || file.media_type === 'show';
    const typeLabel = isTv ? 'TV Show' : 'Movie';
    const qualityLabel = `<span class="quality-badge">${(file.quality||'HD').toUpperCase()}</span><span class="meta-dot">•</span>`;
    
    const sInput = isTv ? `<input type="number" id="tv-season" value="1" min="1" style="width:60px; background:rgba(255,255,255,0.1); color:white; border:none; border-radius:4px; padding:6px; margin-right: 12px; font-weight: bold;">` : '';
    const eInput = isTv ? `<input type="number" id="tv-episode" value="1" min="1" style="width:60px; background:rgba(255,255,255,0.1); color:white; border:none; border-radius:4px; padding:6px; font-weight: bold;">` : '';
    const playCall = isTv ? `playFile(currentFile, document.getElementById('tv-season').value, document.getElementById('tv-episode').value)` : `playFile(currentFile)`;

    hero.innerHTML = `<img src="${file.backdrop_url||file.poster_url||''}" alt="${file.clean_name}"><div class="detail-vignette"></div><div class="detail-info"><h2 class="detail-title">${file.clean_name}</h2><div class="detail-meta">${qualityLabel}<span>${file.year||''}</span><span class="meta-dot">•</span><span>${typeLabel}</span>${file.rating?`<span class="meta-dot">•</span><span class="rating-badge">★ ${file.rating}</span>`:''}</div><p class="detail-synopsis">${file.overview || file.synopsis || 'No synopsis available.'}</p><div class="detail-actions">
        ${isTv ? `<div style="display:flex;gap:8px;align-items:center;margin-bottom:12px;color:#aaa;">Season ${sInput} Episode ${eInput}</div>` : ''}
        <div style="display:flex;gap:12px;">
            <button class="btn-play" onclick="${playCall}"><i data-lucide="play"></i> Play</button>
            <button class="btn-info" onclick="toggleMyList(currentFile.id)"><i data-lucide="${isInMyList(currentFile.id)?'check':'plus'}"></i> My List</button>
        </div>
    </div></div>`;
    
    document.getElementById('detail-extra').innerHTML = ''; // Clear episodes since we don't know them in zero-storage yet
...
async function playFile(file, season = 1, episode = 1) {
    if (!file) return;
    closeDetail();
    currentFile = file;
    _playerRetries = 0;
    ...
    // ── Direct GDrive stream via HF backend proxy ──
    // The backend handles Range requests, so seeking works perfectly.
    // No Playwright, no external providers — 100% your own content.
    const streamUrl = `${API_BASE}/api/stream/${file.id}`;

    try {
        // Use Vidstack media-player if available, else native <video>
        ...
        player.style.cssText = 'width:100%;height:100%;opacity:0;transition:opacity 0.4s ease;';
        ...
        player.addEventListener('can-play', () => {
            ...
            if (saved > 30) {
                ...
                toast(`▶ Resuming from ${Math.floor(saved/60)}m ${Math.floor(saved%60)}s`, 'info');
            }
        }, { once: true });
        ...
        player.addEventListener('ended', () => {
            ...
            if (next) _showNextEpisode(next);
        });

        player.addEventListener('error', () => {
            if (_playerRetries < MAX_RETRIES) {
                _playerRetries++;
                toast(`⚠️ Retrying stream (${_playerRetries}/${MAX_RETRIES})...`, 'warning');
                setTimeout(() => player.setAttribute('src', streamUrl + '?t=' + Date.now()), 1500 * _playerRetries);
            } else {
                _hidePlayerLoading(container);
        ...
        _hidePlayerLoading(container);
        console.error(err);
        _showPlayerError(container, { clean_name: err.message });
    }
}
...
// (Track/subtitle loading removed — streams are proxied directly from GDrive)
// The HF backend /api/stream/{id} handles MKV→WebM spoofing natively.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Infinite scroll for All Content
    window.addEventListener('scroll', () => {
        if (document.getElementById('search-results-row').style.display !== 'none') return;
        const ar = document.getElementById('all-content-row');
        ...
        if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 500) {
            if (!window.isLoadingMore) {
                ...
                currentPage++;
                loadContent(currentPage).finally(() => {
